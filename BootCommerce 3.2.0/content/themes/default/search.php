  ...
  /* MANAGE ADVANCED SEARCH WORDS PARAMS */
   ...
   if(is_array($search_words)){
	   foreach($search_words as $word){
		 ...
		 $search_on .=  isset($_GET['sp_d']) ? 'description LIKE "%'.str_db(trim($word)).'%" or meta_description LIKE "%'.str_db(trim($word)).'%" or ' : '';
         ...
         <aside class="left-sidebar">
          <nav class="accordion_menu_container">
            <section class="header-search-sidebar">
                 ...
                 <?php echo $lang_client_['search']['TEXT_PRODUCTS_FOUND']; ?>: <span class="badge badge-info"><?php echo str_replace(" ","",$get_sp) != '' && (!isset($_GET['sp_t']) && !isset($_GET['sp_d']) && !isset($_GET['sp_mk'])) ? !empty($array_p) ? count($array_p) : 0 : 0; ?></span>
            </section>           
            <label for="sp">
