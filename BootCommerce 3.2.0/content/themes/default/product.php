   ...
   <?php require_once('include/horizontal-categories.php');?> 
     ...
     <section class="row-fluid"><!-- BODY ROW --> 
       <section class="span5 text-center"> 
        <div id="img-preview">
         <img class="lazy" data-original="<?php echo $img_product; ?>" src="<?php echo $img_product; ?>" />
        </div>
           <?php 
                    ...
                    <ul>  
                        ...
                        ?> 
                            <li style="height:70px;overflow:hidden;width:auto;">
                             <a style="height:60px;overflow:hidden;width:auto;" class="thumbnail<?php echo $val['principale'] ? ' active' : ''; ?>" href="#">
                              <img style="cursor:pointer;" data-img="<?php echo str_replace('70x70','600x600',$thumb); ?>" class="lazy" data-original="<?php echo $thumb; ?>" src="<?php echo $thumb; ?>" />
                             </a>
                            </li>
                        <?php
                         }
         ...
         <div class="row-fluid">
          <div class="span4">
           <div class="price-container">
		    <?php echo ($view_prices ? ($offer > 0 ? '<span class="old-price-container">'.str_replace(' ','&nbsp;',$currency_l).num_formatt($price).str_replace(' ','&nbsp;',$currency_r).'</span><span class="product-offer">'.$lang_client_['product']['OFFER_TEXT'].': '.str_replace(' ','&nbsp;',$currency_l).num_formatt($offer).str_replace(' ','&nbsp;',$currency_r).'</span>' : str_replace(' ','&nbsp;',$currency_l).num_formatt($price).str_replace(' ','&nbsp;',$currency_r)) : ''); ?>
             ...
             <?php
			   if($rs['options'] != ''){
				  ...
				  foreach($option_array as $key => $val){
					 echo $val['name'].'<select data-rel="option_'.$rs['id'].'" name="option['.$key.']">';
					 if(!$val['required_option']) echo '<option value=""></option>';
					 ...
					 $arr_option_value = array_msort($arr_option_value, array('value'=>SORT_ASC));
					  foreach($arr_option_value as $key => $val){						   
						 ...
						 echo '<option value="'.$key.'">'.$val['value'].($val['price'] > 0 ? ' '.$val['type'].' '.$currency_l.num_formatt($option_price).$currency_r : '').'</option>';
					  }					 
					 echo '</select>';
				  }
			   }
